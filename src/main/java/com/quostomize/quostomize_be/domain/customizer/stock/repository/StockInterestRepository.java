...
public interface StockInterestRepository extends JpaRepository <StockInterest,Long> {
    ...
    @Query("Select si.priority, sif.stockName, sif.stockPresentPrice, sif.stockImage FROM StockInterest si JOIN FETCH StockInformation sif ON si.stockInformation = sif ORDER BY priority asc")
    List<Object[]> findAllStockInterestDto();
