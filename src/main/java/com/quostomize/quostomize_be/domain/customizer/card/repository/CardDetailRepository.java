...
public interface CardDetailRepository extends JpaRepository<CardDetail, Long> {
    ...
    @Modifying
    @Query("update CardDetail cd set cd.status = :newStatus where cd.cardSequenceId = :cardSequenceId")
    void updateStatus(@Param("newStatus") CardStatus newStatus, @Param("cardSequenceId") Long cardSequenceId);
}