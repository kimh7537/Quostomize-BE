...
public class UpperAndLowerClassification {
    ...
    public void computeByAll(StockInformationRepository stockInformationRepository, S3Service s3Service
        ...
        for (int i = 0; i < 3; i++) {
            if (lowerIndex < computeWithLowerAndUpper.getEntryListLower().size() || upperIndex < computeWithLowerAndUpper.getEntryListUpper().size()) {
                ...
                if (lowerIndex == computeWithLowerAndUpper.getEntryListLower().size()) {
                    ...
                    upperAndLowerClassificationByIndex.getUpperList(entry,stockInformationRepository,recommendResponses,s3Service);
                    upperIndex += 1;
                    ...
                    upperAndLowerClassificationByIndex.getLowerList(entry,stockInformationRepository,recommendResponses,s3Service);
                    lowerIndex += 1;
